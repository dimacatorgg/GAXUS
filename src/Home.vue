...
if(r.params.id=="1"){
  ...
  axios.get(`http://localhost:8000/api/register/?name=${list.value[0][0]}&gmail=${list.value[2][0]}&password=${list.value[1][0]}`,{
    withCredentials:true,
     ...
     );setTimeout(function(){useRouter().replace("/app/")},500) }).catch(err => console.log(err))
  ...
  axios.get(`http://localhost:8000/api/login/?name=${list.value[0][0]}&password=${list.value[1][0]}`,{
   ...
   axios.get(`http://localhost:8000/test/?id=${res.data.message}`,{withCredentials:true}).then(r =>{ localStorage.setItem("user",JSON.stringify(r.data.message[0]));setTimeout(function(){useRouter().replace("/app/")},500);console.log(r.data.message)}).catch(err => console.log(err))
